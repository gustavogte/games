...
# Display buttons
def text_to_button(
    msg, color, buttonx, buttony, buttonwidth, buttonheight, size="small"
):
...
def health_bar(player_health):
    ...
    pygame.draw.rect(
        gameDisplay, player_health_color, (680, 10, player_health * 10, 25)
    )
...
def enemyUFOFire():
    ...
    pygame.draw.line(
        gameDisplay,
        green,
        (enemyUFOX, enemyUFOY + 20),
        (enemyUFOX, enemyUFOY + 500),
        turretWidth,
    )
...
def fire():
    ...
    while fired == True:
        ...
        pygame.draw.line(
            gameDisplay,
            red,
            (mainUFOX - UFOXfiredMovement, mainUFOY - UFOshotStart),
            (mainUFOX - UFOXfiredMovement, mainUFOY - UFOshotEnd),
            turretWidth,
        )
        UFOshotEnd += 20
        UFOshotStart += 20
        ...
        if (
            enemyUFOX + (UFOWidth / 2)
            >= mainUFOX - UFOXfiredMovement
            >= enemyUFOX - (UFOWidth / 2)
            and mainUFOY - UFOshotStart == enemyUFOY
        ):
            newScore += 1
